...
import { MODES,modeOf,SEATS,teamOf } from './rules.js'
...
function renderMatchScore(){const game=state.game;if(!game)return;if(game.seats?.length>2){$('#match-score').textContent='';return}if(state.mode==='hotseat'){$('#match-score').textContent=`Race to ${state.raceTo||MATCH_TARGET} · ${game.nameOf('a')} ${state.matchScore.a}–${state.matchScore.b} ${game.nameOf('b')}`;return}const theirs=game.me==='a'?'b':'a';$('#match-score').textContent=`Race to ${state.raceTo||MATCH_TARGET} · ${state.matchScore[game.me]}–${state.matchScore[theirs]}`}
...
async function shareResult(){const title=$('#result-title').textContent,text=`${title} · ${$('#result-summary').textContent} · P.U.S.H. Pool`;try{if(navigator.share)await navigator.share({title:'Pool Masters',text});else await navigator.clipboard.writeText(text);toast('Result shared')}catch{}}
...
async function shareReplay(){
 ...
 try{
  ...
  if(navigator.share&&matchMedia('(pointer:coarse)').matches)await navigator.share({title:'A P.U.S.H. Pool shot',text:'Watch this shot',url:url.href})
...
async function openReplay(link){
 ...
 $('#versus').innerHTML='<span style="grid-column:1/-1;text-align:center"><b>Shared shot</b><small>Recorded in P.U.S.H. Pool</small></span>'
...
async function startHotSeat(mode=chosenMode(),party=null){
 state.game?.destroy();state.mode='hotseat';state.room=null;state.opponent={name:'Player 2'};state.matchScore={a:0,b:0}
 // a party (three to six players on one device, P.U.S.H. Pool) always plays one shot a turn; the seats take the turns in order
 const house=party?{...houseRules(),oneShot:true}:houseRules()
 const names=party?Object.fromEntries(party.seats.map((p,i)=>[p,`Player ${i+1}`])):{a:'Player 1',b:'Player 2'}
 showGame();state.raceTo=house.race;$('#game').classList.add('focus');history.replaceState({},'',location.pathname)
 $('#room-label').textContent=party?`${party.seats.length} players${party.teams?' in two teams':''} · ${MODES[mode].label}${houseTag(house)}`:`Two players · ${MODES[mode].label}${houseTag(house)}`
 $('#versus').innerHTML=party?'':'<span><b>Player 1</b><small>Break</small></span><i>vs</i><span><b>Player 2</b><small>Pass the device</small></span>'
 $('#practice-record').hidden=true
 state.game=new PoolGame({mode,house,obstacles:chosenObstacles(),renderer:await ensureRenderer(),surface:$('.canvas-wrap'),status:$('#game-status'),groupStatus:$('#groups'),callout:$('#callout'),power:$('#power'),powerOut:$('.shot-controls output'),shoot:$('#shoot'),jumpBtn:$('#jump'),spinPad:$('#spin'),moveCue:$('#move-cue'),changePocket:$('#change-pocket'),sfx,onReplay:paintReplayBar,aimSensitivity:tablePrefs.aimSensitivity,prefs:tablePrefs,host:true,practice:true,hotSeat:true,names,send:()=>{},...(party?{seats:party.seats,teams:party.teams}:{}),
  onTurn:name=>toast(`${name}’s turn`),
  onFinish:result=>{music.duck();sfx.result(true);if(party)showPartyResult(result);else{showRackResult(result);$('#next-rack').hidden=false}}})
 renderMatchScore()
 $('.call-actions').hidden=true;$('#room-sidebar').hidden=true
}
function showPartyResult(result){
 const game=state.game,seats=game.seats,winner=result.winner
 const teamNote=game.teams?` (${seats.filter(p=>teamOf(seats,p)===teamOf(seats,winner)).map(p=>game.nameOf(p)).join(' + ')})`:''
 $('#result-title').textContent=`${game.nameOf(winner)} wins${teamNote}`
 $('#result-summary').textContent=seats.map(p=>`${game.nameOf(p)} ${game.score[p]}`).join(' · ')
...
function openHouse(){
 ...
 $('#house-jumps').checked=h.jumps;$('#house-cannon').checked=h.cannon;$('#house-oneshot').checked=h.oneShot
 $('#house-summary').textContent=houseIsDefault(h)?'Standard rules.':describeHouse(h)
 $('#house-dialog').showModal()
}
function saveHouse(){
 const h=normalizeHouse({race:Number($('#house-race').value),ballInHand:$('#house-bih').value,breaker:$('#house-breaker').value,straightTo:Number($('#house-straight').value),jumps:$('#house-jumps').checked,cannon:$('#house-cannon').checked,oneShot:$('#house-oneshot').checked})
