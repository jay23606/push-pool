...
import {R,PR,MINX,MAXX,MINY,MAXY,POCKETS,tableSize,W,H} from './table.js'
...
export class PoolGame{
 ...
 jumpAllowed(){
  if(isPush(this.mode))return Boolean(this.push?.[this.turn]?.powers.jump)&&(this.score?.[this.turn]||0)>=powerCost('jump',1)&&!this.replay
 ...
 takeShot(){if(!this.canAim()||!this.aiming)return;this.placing=null;if(!this.drill&&this.eightGame()&&this.guide().hit?.k==='eight'&&!this.canCallEight()){this.calledPocket=null;this.aiming=false;this.flash(this.eightBlockedMessage());return}const cannonOn=isPush(this.mode)&&this.armedItem==='cannon'&&Boolean(this.push?.[this.turn]?.items.includes('cannon')),s=shotSpeed(+this.power.value)*(cannonOn?CANNON_MULT:1),vx=Math.cos(this.angle)*s,vy=Math.sin(this.angle)*s,spin=[this.spin.a,this.spin.b];if(this.mode!==undefined&&isPush(this.mode)&&this.armed?.spin){const k=1+powerLevel('spin',this.armed.spin).amount;spin[0]*=k;spin[1]*=k};if(!this.drill&&!this.chal&&!(this.jumpAllowed()&&this.jumpOn))this.lastCoach={before:this.balls.map(b=>({...b})),shot:{angle:this.angle,power:+this.power.value,spin},group:this.group(this.me),mode:this.mode,player:this.me,breakShot:this.breakShot,result:null,replays:null};this.aiming=false;this.sfx?.cue(+this.power.value/100);
  ...
  if(jump&&this.host&&isPush(this.mode))this.payJump()
  const armed=isPush(this.mode)&&Object.keys(this.armed||{}).length?{...this.armed}:null;this.armed={}
  ...
  this.startShot();if(this.cannonShot){this.balls[0].m=CANNON_MASS;this.cannonShot=false}
  if(this.host)strike(this.balls[0],vx,vy,spin[0],spin[1],jump);else this.send({t:'shot',vx,vy,spin,place:this.pendingPlace,called,...(jump?{jump:true}:{}),...(armed?{powers:armed}:{}),...(armed?.trail?{tv:this.trailVariant||'ice'}:{}),...(cannonOn?{cannon:true}:{})});this.pendingPlace=null}
 startShot(){if(this.chal&&this.chal.startedAt==null)this.chal=startClock(this.chal,performance.now());this.shots??={a:0,b:0};this.shots[this.turn]=(this.shots[this.turn]||0)+1;this.placed=false;this.potted=[];this.firstObjectPotted=null;this.scratch=false;this.firstHit=null;this.before=this.group()?this.remaining(this.group()):null;this.lowest=lowestBall(this.balls);this.railHit=false;this.pocketOf={};this.railBalls=new Set();this.cueRailFirst=false;this.phase='roll';this.noteRecording(true)
  if(isPush(this.mode))this.shotStarted()}
 receive(m){
  ...
  if(m.t==='use'&&this.host)return this.applyUse('b',m.id,m.arg)
 ...
 receiveShot(m){
  ...
  if(jump&&isPush(this.mode))this.payJump()
  if(isPush(this.mode)&&m.powers)this.applyArmed(m.powers,{trail:m.tv||'ice'})
  if(isPush(this.mode)&&m.cannon&&this.push?.[this.turn]?.items.includes('cannon'))this.payCannon()
  if(this.shotFx?.nudge){this.doNudge(m.vx,m.vy,m.spin||[0,0]);return}
  this.startShot();if(this.cannonShot){this.balls[0].m=CANNON_MASS;this.cannonShot=false}
  strike(this.balls[0],m.vx,m.vy,m.spin?.[0]||0,m.spin?.[1]||0,jump)
 }
 ...
 pocketList(){
  ...
  if(!this._pockets||this.fx!==this._pocketFx||this._pocketPR!==PR||this._pocketObs!==this.push?.obstacles){
   ...
   this._pockets=isPush(this.mode)?[...base,...this.bonusHoles().map(o=>({x:o.x,y:o.y,r:o.r}))]:bp?[...base,{x:bp.x,y:bp.y,r:bp.r}]:base
  }
 ...
 pushAfterShot(shooter,v){
  ...
  let rolling=false
  for(const bl of r.blasts||[]){blast(this.balls,bl,{radius:bl.radius,power:bl.power});rolling=true}
  for(const sp of r.spews||[])this.balls.push(...scatterAround(this.balls,sp.count,sp.x,sp.y,Math.random))
  if(r.barf)this.barf()
  ...
  // an eruption sets balls rolling: let them settle, and whatever drops scores for the player about to shoot
  if(rolling&&!this.tossing)this.tossStart()
 }
 // the pockets give back some of what they last swallowed: real balls come back, dummies are made anew
 barf(){
  const log=this.pocketLog||[];if(!log.length)return
  const pockets=[...new Set(log.map(e=>e.p))],p=pockets[Math.floor(Math.random()*pockets.length)]
  const mine=log.filter(e=>e.p===p),take=mine.slice(-Math.min(mine.length,1+Math.floor(Math.random()*5)))
  this.pocketLog=log.filter(e=>!take.includes(e))
  const [px,py]=POCKETS[p],dx=W/2-px,dy=H/2-py,d=Math.hypot(dx,dy)||1
  take.forEach((e,i)=>{
 ...
 // A tossed or blasted table settles before anything is scored: the flag, the cue ball's place, and a clean slate for the pots.
 tossStart(){
  const cue=this.balls[0]
  this.tossing=true;this.tossCue={x:cue.x,y:cue.y}
  this.potted=[];this.firstObjectPotted=null;this.scratch=false;this.firstHit=null;this.pocketOf={};this.railBalls=new Set();this.phase='roll';this.acc=0
 }
 applyPick(player,id){
 ...
 pickPower(id){
  ...
  if(!isPush(this.mode)||!this.push?.offers||this.turn!==me||this.spectator)return
 ...
 syncObstacles(){
  if(!isPush(this.mode))return
  const list=this.push?.obstacles||[],sig=JSON.stringify(list)
  if(sig!==this.obsSig){this.obsSig=sig;setObstacles(list)}
 }
 startPlacing(id){
  const me=this.hotSeat?this.turn:this.me
  if(!isPush(this.mode)||!this.push||this.turn!==me||!this.canControl()||this.ballInHand||!(isPlaceable(id)||isTossable(id))||!this.push[me].items.includes(id))return false
 ...
 applyToss(player,item,target){
  ...
  if(eff.kind==='piggy'){
   ...
   this.push={...this.push,obstacles:[...(this.push.obstacles||[]),pig]};this.syncObstacles();this.dispense(pig);this.sync();this.flash('Piggy bank');return
  }
  const at=skidTo(spot,spot.dir,item,ctx.bounds)
  this.tossStart()
  blast(this.balls,at,{radius:eff.radius,power:eff.power});this.flash('BOOM');this.sync()
 }
 // A piggy bank spills a few gems each time something hits it (and when it is thrown), then goes when it is empty.
 dispense(pig){
  const list=this.push.obstacles,i=list.findIndex(o=>o===pig||(o.piggy!==undefined&&o.x===pig.x&&o.y===pig.y));if(i<0)return
  const cur=list[i],n=Math.min(cur.piggy,1+Math.floor(Math.random()*4)),gems=[]
  for(let k=0;k<n;k++){
   const at=freeSpot(this.balls,[...(this.push.pickups||[]),...gems],{minx:Math.max(MINX,cur.x-60),maxx:Math.min(MAXX,cur.x+60),miny:Math.max(MINY,cur.y-60),maxy:Math.min(MAXY,cur.y+60)},Math.random)
   if(at)gems.push({kind:'gem',v:rollGem(Math.random),x:at[0],y:at[1],ttl:2})
  }
  const left=cur.piggy-n,obstacles=left>0?list.map((o,j)=>j===i?{...o,piggy:left}:o):list.filter((_,j)=>j!==i)
  this.push={...this.push,obstacles,pickups:[...(this.push.pickups||[]),...gems].slice(-40)};this.syncObstacles()
 }
 piggyCheck(){
  const list=this.push?.obstacles;if(!list?.some(o=>o.piggy>0))return
  const seen=this.piggyTouched??=new Set()
  for(const o of list){
   if(!(o.piggy>0))continue
   for(const b of this.balls){
    if(!b.on||Math.hypot(b.vx,b.vy)<4||Math.hypot(b.x-o.x,b.y-o.y)>R+o.r+2)continue
    const key=o.x+','+o.y+':'+b.n;if(seen.has(key))continue
    seen.add(key);this.dispense(o);this.flash('Gems!');return
   }
  }
 }
 resolveToss(){
  ...
  this.tossing=false;this.shotFx=null;this.trailPts=null;delete this.balls[0].m;this.balls.forEach(clearMotion);this.phase='aim'
 ...
 cycleArm(id){
  const me=this.hotSeat?this.turn:this.me
  if(!isPush(this.mode)||!this.push||this.turn!==me||!this.canControl()||!isArmable(id))return
 ...
 releaseBall({n,x,y}){
  ...
  this.placeNear(b,x,y);b.on=true;clearMotion(b)
 }
 // put a ball at the free spot nearest (x,y), spiralling outward
 placeNear(b,x,y){
  for(let i=0;i<40;i++){
 ...
 // Pop powder lights the next shot, and every shot leaves a snapshot behind for a Mulligan (taken after the powers were paid for,
 ...
 powderPop(a,b){
  ...
  this.powders=(this.powders||0)+1;blast(this.balls,{x:(a.x+b.x)/2,y:(a.y+b.y)/2},{radius:POWDER_RADIUS_R*R,power:POWDER_FORCE})
 }
 // Things done from the panel that are not a shot: the host does them, a guest asks (t:'use').
 requestUse(id,arg){
  const me=this.hotSeat?this.turn:this.me
  if(!isPush(this.mode)||this.spectator||this.replay)return
  if(this.host)this.applyUse(me,id,arg);else this.send({t:'use',id,arg})
 }
 applyUse(player,id,arg){
  if(!this.push||this.tossing||this.phase!=='aim'||this.over)return
  if(id==='mulligan')return this.applyMulligan(player)
  if(this.turn!==player)return
  if(id==='poppowder'){
   const mine=this.push[player],next=useItem(mine,'poppowder','before');if(next===mine)return
   this.push={...this.push,[player]:{...next,powder:true}};this.flash('Pop powder · the next shot');this.sync()
  }else if(id==='tilt'){
   const level=this.push[player].powers.tilt||0;if(!level)return
   const dir={up:[0,-1],down:[0,1],left:[-1,0],right:[1,0]}[arg];if(!dir)return
   const r=payPower(this.push,this.score,player,'tilt',level,'before');if(!r.ok)return
   this.score=r.score
   const k=powerLevel('tilt',level).shift*TILT_IMPULSE
   this.tossStart();for(const b of this.balls)if(b.on){b.vx+=dir[0]*k;b.vy+=dir[1]*k}
   this.flash('Tilt');this.sync()
  }
 }
 // A Mulligan takes the table back to how it was before the last shot, whoever took it. What that shot cost stays paid,
 // and the Mulligan is used up. Whoever shot it shoots it again.
 applyMulligan(user){
  const u=this.undo;if(!u||!this.push[user].items.includes('mulligan'))return
  this.balls=u.balls.map(b=>({...b}));this.score=u.score;this.turn=u.turn;this.ballInHand=u.ballInHand;this.breakShot=u.breakShot
  this.push={...u.push,[user]:useItem(u.push[user],'mulligan','after')};this.undo=null;this.shotFx=null
  this.syncObstacles();this.flash('Mulligan · the shot is rewound');this.sync()
  if(this.practice&&!this.hotSeat&&!this.over&&this.turn==='b')setTimeout(()=>this.aiShot(),650)
 }
 // A nudge is a small tap of the cue ball that is not a shot: no foul, and the turn stays with you.
 doNudge(vx,vy,spin){
  const cue=this.balls[0],len=Math.hypot(vx,vy)||1,speed=shotSpeed(NUDGE_POWER*this.shotFx.nudge.reach)
  const {nudge:_n,...rest}=this.shotFx;this.shotFx=Object.keys(rest).length?rest:null
  this.tossStart();strike(cue,vx/len*speed,vy/len*speed,spin[0]||0,spin[1]||0,false)
 }
 // the cannon: the cue ball goes out as a heavy ball at the hardest shot there is
 payCannon(){this.push={...this.push,[this.turn]:useItem(this.push[this.turn],'cannon','before')};this.cannonShot=true;this.flash('CANNON!')}
 toggleCannon(){
  const me=this.hotSeat?this.turn:this.me
  if(!isPush(this.mode)||!this.canControl()||this.turn!==me||!this.push[me].items.includes('cannon'))return
 ...
 collect(){
  if(!isPush(this.mode)||!this.push?.pickups?.length)return
 ...
 sub(dt){
  ...
  for(const b of this.balls){
   ...
   integrate(b,dt)
   for(let p=0;p<pockets.length;p++){const q=pockets[p];if(!airborne(b)&&Math.hypot(b.x-q.x,b.y-q.y)<q.r*(p<6?scale:1)){if(p>=6&&isPush(this.mode)){this.sinkBonus(b,p-6);break}b.on=false;if(b.k==='cue')this.scratch=true;else{this.potted.push(b);if(isPush(this.mode)){(this.pocketLog??=[]).push({p,n:b.n,k:b.k});if(this.pocketLog.length>30)this.pocketLog.shift()};(this.pocketOf??={})[b.n]=p;if(!this.firstObjectPotted&&(b.k==='solid'||b.k==='stripe'))this.firstObjectPotted=b}if(b.k==='eight')this.eightPocket=p;this.flash(this.pottedMessage(b));break}}
   if(!b.on)continue
   if(railBounce(b)){
  ...
  for(let i=0;i<this.balls.length;i++)for(let j=i+1;j<this.balls.length;j++){
   ...
   if(ballCollide(a,b)){
    // the first ball the cue ball touched; a real ball replaces a dummy (dummies count as contact, but are never 'wrong')
    {const o=a.k==='cue'?b:b.k==='cue'?a:null;if(o&&(!this.firstHit||(this.firstHit.k==='dummy'&&o.k!=='dummy')))this.firstHit=o}
 ...
 resolve(){
  ...
  this.shotFx=null
  if(isPush(this.mode))this.pushAfterShot(shooter,v)
  this.newTwist()
  this.sync()
  if(this.practice&&!this.hotSeat&&!this.over&&this.turn==='b')setTimeout(()=>this.aiShot(),650)
 }
 aiShot(){
  ...
  const armed={}
  if(isPush(this.mode)&&this.push&&this.turn==='b'){
 ...
 scoreMessage(v,shooter){
  ...
  if(v.credited.length){
   ...
   if(!isPush(this.mode)&&v.credited.some(c=>c.points>1)){this.flash(`BONUS · +${v.credited.reduce((n,c)=>n+c.points,0)} · ${v.score[shooter]} of ${(this.scoreTarget||targetFor(this.mode))}`);return}
 ...
 updateScoreHud(){
  ...
  const label=isPush(this.mode)?'P.U.S.H. POOL':this.mode==='bank'?'BANK POOL':this.mode==='straight'?'STRAIGHT POOL':this.mode==='chaos'?`CHAOS POOL · ${twistName(this.fx)}`:'ONE-POCKET'
  ...
  if(this.moveCue)this.moveCue.hidden=!(live&&this.placed)
  if(this.changePocket)this.changePocket.hidden=true
  const rule=isPush(this.mode)?'pot for points, grab gems and items':this.mode==='bank'?'bank it off a cushion':this.mode==='straight'?'pot any ball, a foul costs a point':this.mode==='chaos'?'pot any ball, mind the twist':'sink it in the ringed pocket'
 ...
 updateHud(){
  this.paintJump()
  if(isPush(this.mode))renderPushPanel(this)
