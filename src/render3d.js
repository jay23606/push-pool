...
function ballTexture(THREE,kind,n){
 ...
 const g=c.getContext('2d'),white='#f7f4e9',col=COLORS[n]
  if(kind==='dummy'){g.fillStyle=dummyColor(n);g.fillRect(0,0,TW,TH)}
 ...
 const buildObstacles=list=>{
  ...
  for(const o of list){
   if(o.t==='bumper'){const m=new THREE.Mesh(new THREE.CylinderGeometry(o.r,o.r,16,28),new THREE.MeshStandardMaterial({color:o.vol!==undefined?'#c2410c':o.piggy?'#f472b6':'#9aa0a3',emissive:o.vol!==undefined?'#ff5a1f':'#000000',emissiveIntensity:o.vol!==undefined?.5:0,metalness:.7,roughness:.3}));m.position.set(tx(o.x),8,tz(o.y));obsGroup.add(m)}
    ...
    disc.position.set(tx(o.x),1.2,tz(o.y));ring.rotation.x=Math.PI/2;ring.position.set(tx(o.x),2.2,tz(o.y));icon.position.set(tx(o.x),R*2.6,tz(o.y));for(const q of [disc,ring,icon])obsGroup.add(q)
   }
   else if(o.t==='slick'){const c={ice:'#96dcff',electric:'#ffe65a',sand:'#dcb46e',plasma:'#c85aff'}[o.variant]||'#ffffff',m=new THREE.Mesh(new THREE.CircleGeometry(o.r,40),new THREE.MeshBasicMaterial({color:c,transparent:true,opacity:.4,depthWrite:false}));m.rotation.x=-Math.PI/2;m.position.set(tx(o.x),.9,tz(o.y));obsGroup.add(m)}
   else if(o.t==='blackhole'){const m=new THREE.Mesh(new THREE.CircleGeometry(o.r,48),new THREE.MeshBasicMaterial({color:'#4a1c8c',transparent:true,opacity:.35,depthWrite:false})),core=new THREE.Mesh(new THREE.CircleGeometry(12,24),new THREE.MeshBasicMaterial({color:'#000000'}));for(const q of [m,core]){q.rotation.x=-Math.PI/2;obsGroup.add(q)}m.position.set(tx(o.x),.9,tz(o.y));core.position.set(tx(o.x),1.2,tz(o.y))}
