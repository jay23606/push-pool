...
test('table preferences reject stale or invalid values',()=>{
 const p=loadTablePrefs({getItem:k=>({
  'push-pool:table-size':'12','push-pool:felt':'#bad',
  'push-pool:cue':'neon','push-pool:lighting':'laser','push-pool:rails':'plastic'}[k]??null)})
...
test('an out-of-range aim sensitivity falls back to the default rather than clamping silently',()=>{
 for(const bad of ['0','.05','1.4','not-a-number',null])
  assert.equal(loadTablePrefs({getItem:k=>k==='push-pool:aim-sensitivity'?bad:null}).aimSensitivity,.3,`${bad} should fall back`)
 assert.equal(loadTablePrefs({getItem:k=>k==='push-pool:aim-sensitivity'?'.65':null}).aimSensitivity,.65)
})
test('table preferences are normalized before persisting',()=>{
 ...
 assert.equal(p.size,9);assert.equal(store.getItem('push-pool:cue'),'ebony')
})
...
test('the follow-the-shot camera is on unless the player turned it off, and only "0" turns it off',()=>{
 ...
 assert.equal(loadTablePrefs({getItem:k=>k==='push-pool:shot-cam'?'0':null}).shotCam,false)
 for(const junk of ['1','','off','false','x'])assert.equal(loadTablePrefs({getItem:k=>k==='push-pool:shot-cam'?junk:null}).shotCam,true)
 ...
 assert.equal(store['push-pool:shot-cam'],'0')
})

test('the shot camera height is kept in range and falls back to the default when it is not',()=>{
 const load=v=>loadTablePrefs({getItem:k=>k==='push-pool:eye-height'?v:null}).eyeHeight
 ...
 assert.equal(store['push-pool:eye-height'],120)
})
